...
def main(url):
 ...
 try:
		...
		if "azzatssins" in get_soe:
		 ...
		 get_soure = requests.get(url+"/alfa.php",headers=headers, timeout=3, verify=False, allow_redirects=False).text
   		 if "~ ALFA TEaM Shell" in get_soure:
		   print '[OK!]' + url+'/wso.php'
		   se = open('content.txt', 'a')
		   se.write(url+'/alfa.php\n')
		   se.close()
		 else:
		  get_soue = requests.get(url+"/wso.php",headers=headers, timeout=3, verify=False, allow_redirects=False).text
		  if "- WSO 2.5" in get_soue:
		   print '[OK!]' + url+'/wso.php'
		   se = open('content.txt', 'a')
		   ...
		   if "Chitoge kirisaki" in get_source:
		    ...
		    if "<title>MARIJUANA</title>" in get_source or "0x5a455553.github.io/MARIJUANA/icon.png" in get_source:
		     ...
		     se.close()
		       ...
		       get_s = requests.get(url+"/shell.php",headers=headers, timeout=3, verify=False, allow_redirects=False).text
		       if "Mini Shell" in get_s or "Mini Uploader" in get_s:
		        print '[OK!]' + url+'/shell.php'
		        se = open('content.txt', 'a') 
		        se.write(url+'/shell.php\n') 
		        se.close()
		       else:
		          get_s = requests.get(url+"/webshell.php",headers=headers, timeout=3, verify=False, allow_redirects=False).text
		          if 'Path: /' in get_s in 'path: /' in get_s:
		           print '[OK!]' + url+'/webshell.php'
		           se = open('content.txt', 'a') 
		           se.write(url+'/webshell.php\n') 
		           se.close()
		          else:		              
		              get_s = requests.get(url+"/rootshell.php",headers=headers, timeout=3, verify=False, allow_redirects=False).text
		              if 'ROOT SHELL EXECUTOR' in get_s:
